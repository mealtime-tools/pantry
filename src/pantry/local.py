...
def as_result(product: Product) -> dict:
    ...
    # Sodium is milligrams, and it is the one nutrient most records predate:
    # a defaulted 0 would read as a sodium-free product rather than an
    # unknown one, so it is carried only when the record holds it.
