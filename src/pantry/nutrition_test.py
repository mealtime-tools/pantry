...
def test_kilojoules_are_divided_by_the_published_ratio() -> None:
    ...
    # A definition, not a measurement, so the division is exact throughout.
...
def test_a_sodium_row_is_still_read_from_its_own_name() -> None:
    assert panel_from_rows([("Sodium", "400mg")]) == {"sodium": Decimal("0.4")}
...
def test_a_microgram_figure_converts_instead_of_becoming_grams(
    ...
    assert panel == {"cholesterol": Decimal("0.0000012")}
...
def test_a_macro_needs_no_unit_and_every_other_nutrient_does() -> None:
    ...
    assert panel_from_rows([("Protein", "8.5")]) == {"protein": Decimal("8.5")}
...
@pytest.mark.parametrize(
    ("row", "expected"),
    [
        (("Calcium", "120mg"), {"calcium": Decimal("0.12")}),
        (("Iron", "3.2mg"), {"iron": Decimal("0.0032")}),
        (("Vitamin D", "1.2µg"), {"vitamin_d": Decimal("0.0000012")}),
        (("Folic Acid", "194µg"), {"folic_acid": Decimal("0.000194")}),
        (
            ("Pantothenic-Acid", "1.4mg"),
            {"pantothenic_acid": Decimal("0.0014")},
        ),
    ],
)
def test_a_vitamin_or_mineral_row_is_read_from_its_wire_name(
    row: tuple[str, str], expected: dict[str, Decimal]
) -> None:
...
def test_a_longer_vitamin_name_is_not_claimed_by_a_shorter_one() -> None:
    ...
    assert panel_from_rows([("Vitamin B12", "2.4mcg")]) == {
        "vitamin_b12": Decimal("0.0000024")
    }
    assert panel_from_rows([("Vitamin B6", "0.5mg")]) == {
        "vitamin_b6": Decimal("0.0005")
    }
...
def test_the_derived_rows_leave_the_hand_written_order_alone() -> None:
    ...
    assert panel_from_rows([("- Saturated", "0.6g")]) == {
        "saturated_fat": Decimal("0.6")
    }
    assert panel_from_rows([("Fat, Total", "3.6g")]) == {"fat": Decimal("3.6")}
    assert panel_from_rows([("Monounsaturated", "1.2g")]) == {
        "monounsaturated_fat": Decimal("1.2")
    }
